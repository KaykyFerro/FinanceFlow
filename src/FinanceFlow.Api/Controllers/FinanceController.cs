...
public sealed class FinanceController(FinanceFlowDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> Summary(int? year, int? month, CancellationToken ct)
    {
        ...
        var investment = accounts.Where(x => x.Type == AccountType.Investment).Sum(x => x.Balance) + allTx.Where(x => x.Type == TransactionType.Income && accounts.Any(a => a.Id == x.AccountId && a.Type == AccountType.Investment)).Sum(x => x.Amount) - allTx.Where(x => x.Type == TransactionType.Expense && accounts.Any(a => a.Id == x.AccountId && a.Type == AccountType.Investment)).Sum(x => x.Amount);
    ...
    public async Task<IActionResult> CreateTransaction(TransactionRequest request, CancellationToken ct)
    {
        ...
        var account = await db.Accounts.SingleOrDefaultAsync(x => x.Id == request.AccountId && x.UserId == userId, ct);
        if (account is null) return BadRequest(new { message = "Conta inválida." });
        ...
        return Created($"/api/finance/transactions/{item.Id}", item);
    }
    ...
    {
        var userId = GetUserId();
        var item = await db.Transactions.SingleOrDefaultAsync(x => x.Id == id && x.UserId == userId, ct);
    ...
    {
        var userId = GetUserId();
        if (string.IsNullOrWhiteSpace(request.Name)) return BadRequest(new { message = "Nome da conta é obrigatório." });
    ...
    public async Task<IActionResult> UpdateAccount(Guid id, AccountRequest request, CancellationToken ct)
    {
        ...
        if (item is null) return NotFound();
        item.ConfigureYield(item.YieldType, item.YieldPercentage);
        var sql = "UPDATE \"Accounts\" SET \"Institution\"={0}, \"Name\"={1}, \"Type\"={2}, \"Balance\"={3} WHERE \"Id\"={4} AND \"UserId\"={5}";
        await db.Database.ExecuteSqlRawAsync(sql, request.Institution ?? "", request.Name, Enum.Parse<AccountType>(request.Type, true), request.Balance, id, GetUserId());
        return Ok();
    }
    ...
    public async Task<IActionResult> DeleteAccount(Guid id, CancellationToken ct)
    {
        var item = await db.Accounts.SingleOrDefaultAsync(x => x.Id == id && x.UserId == GetUserId(), ct);
        if (item is null) return NotFound();
        if (await db.Transactions.AnyAsync(x => x.AccountId == id, ct)) return Conflict(new { message = "Não é possível excluir uma conta que possui transações." });
    ...
    {
        var userId = GetUserId();
        if (string.IsNullOrWhiteSpace(request.Name)) return BadRequest(new { message = "Nome da categoria é obrigatório." });
    ...
    public async Task<IActionResult> DeleteCategory(Guid id, CancellationToken ct)
    {
        ...
        if (item is null) return NotFound();
        if (await db.Transactions.AnyAsync(x => x.CategoryId == id, ct)) return Conflict(new { message = "A categoria possui transações e não pode ser excluída." });
    ...
    public async Task<IActionResult> CreateBudget(BudgetRequest request, CancellationToken ct)
    {
        ...
        if (existing is not null) { existing.Update(request.CategoryId, month, request.LimitAmount); }
        else db.Budgets.Add(new Budget(userId, request.CategoryId, month, request.LimitAmount));
    ...
    {
        if (!await db.Accounts.AnyAsync(x => x.UserId == userId, ct))
        {
            db.Accounts.AddRange(
                new Account(userId, "Nubank", "Nubank", AccountType.Checking),
                new Account(userId, "Caixa", "Caixinha", AccountType.Savings),
                new Account(userId, "Santander", "Santander", AccountType.Checking),
                new Account(userId, "Carteira", "Dinheiro", AccountType.Wallet));
        }
