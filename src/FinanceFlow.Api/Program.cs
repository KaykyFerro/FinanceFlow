...
if (int.TryParse(railwayPort, out var port) && port > 0) builder.WebHost.ConfigureKestrel(options => options.Listen(IPAddress.Any, port));
...
if (string.IsNullOrWhiteSpace(connectionString)) throw new InvalidOperationException("DATABASE_URL ou ConnectionStrings:DefaultConnection não está configurada.");
...
builder.Services.AddHttpClient("Resend", client => { client.BaseAddress = new Uri("https://api.resend.com/"); client.Timeout = TimeSpan.FromSeconds(20); client.DefaultRequestHeaders.Accept.ParseAdd("application/json"); });
...
static string ToNpgsqlConnectionString(string value)
{
    ...
    var cs = new NpgsqlConnectionStringBuilder { Host = uri.Host, Port = uri.IsDefaultPort ? 5432 : uri.Port, Database = uri.AbsolutePath.TrimStart('/'), Username = Uri.UnescapeDataString(userInfo[0]), Password = userInfo.Length > 1 ? Uri.UnescapeDataString(userInfo[1]) : string.Empty, SslMode = SslMode.Require };
